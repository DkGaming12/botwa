...
const katadidik = JSON.parse(fs.readFileSync('./database/json/katadidik.json'))
...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Didik\n'
            + 'ORG:Owner Gans;\n'
            + 'TEL;type=CELL;type=VOICE;waid=6287867336930:Didik\n'
            + 'END:VCARD'
...
limitt = 9999999999
memberLimit = 2
ban = ["62867676767@s.whatsapp.net"]
premium = ["6287867336930@s.whatsapp.net"]
...
async function starts() {
	...
	dms.on('chat-update', async (mek) => {
		try {
			...
			const BotWea2 = ["087867336930@s.whatsapp.net"] // ubah aja gapapa
			const Didik = mek.message.conversation
			...
			mess = {
				...
				success: '*SUCCES...*',
				error: {
					stick: ' *MAAF TERJADI KESALAHAN SAAT CONVERT TO STICKER*',
					Iv: '*MAAF LINK TIDAK VALID!!*'
				},
				only: {
					group: '*MAAF FITUR INI HANYA BISA DIGUNAKAN DIDALAM GROUP!*',
					benned: '*MAAF NOMOR KAMU DI BAN SILAHKAN HUB OWNER UNTUK UNBAN!*',
					ownerG: '*MAAF COMMAND INI KHUSUS OWNER GROUP!*',
					ownerB: '*MAAF COMMAND INI KHUSUS OWNER BOT* ',
					premium: '*FITUR TERKUNCI!!! KHUSUS PREMIUM SILAHKAN HUB OWNER!!!*',
					userB: `*Hallo Kak* ${pushname2} *Kayanya Kamu Blm Terdaftar Sebagai Teman Didik BOT*\n*Ketik ${prefix}daftar nama|umur*\n*Contoh ${prefix}daftar didik|17*`,
			...
			const ownerNumber = ["6287867336930@s.whatsapp.net"] // owner number ubah aja
		...
		case prefix+'daftar':
			...
			//if (!isPublic) return reply(mess.only.publikG)
				...
				case prefix+'info':
					...
					user.push(sender)
					uptime = process.uptime()
					teks = `➽ *Nama Bot* : ${me.name}\n➽ *Owner Bot* : wa.me/6287867336930\n➽ *prefix* : | ${prefix} |\n➽ *Total Block* : ${blocked.length}\n➽ *Aktif Sejak* : ${kyun(uptime)}\n\n➽ *Total Pengguna* : ${user.length} User\n➽ *Instagram* : https://www.instagram.com/dkgaming12\n➽ *Special Thanks To* :\n\n➽ Allah SWT`
            ...
            break  
				...
				case prefix+'owner':
                 ...
                 dms.sendMessage(from, 'Kalo mau disave chat ae ntar disave back :3',text, { quoted: mek} )
                 break
                case prefix+'fitnah':
                 if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
...
//akhir kreator
		    ...
		    case prefix+'darkjokes':
				...
				await limitAdd(sender) 
				break  
			case prefix+'katadidik':
				if (isBanned) return reply(mess.only.benned)    
				if (!isUser) return reply(mess.only.userB)
				//if (!isPublic) return reply(mess.only.publikG)
				hasil = katadidik[Math.floor(Math.random() * (katadidik.length))]
				dms.sendMessage(from, '*'+hasil+'*', text, {quoted: mek})
				await limitAdd(sender)
				break 
		    ...
		    case prefix+'map':
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔═══✪ Tag By *${pushname2}* ✪══`+ teks +'╚═══〘 DIDIK BOT 〙═══', members_id, true)
					break
			    ...
			    //if (!isPublic) return reply(mess.only.publikG)
					...
					mentions(`╔══〘  *${body.slice(12)}*  〙✪══`+teks+'╚═〘 DIDIK BOT 〙', members_id, true)
					break
			    case prefix+'kbbi':
			    if (isBanned) return reply(mess.only.benned)    
			    if (!isUser) return reply(mess.only.userB)
				...
				case prefix+'clearall':
					...
					reply(`*_Sukses delete all chat DIDIK BOT_*`)
					break
                                case prefix+'bcgc':
					dms.updatePresence(from, Presence.composing) 
					if (!isOwner) return reply(mess.only.ownerB)
				...
				case prefix+'bc':
					...
					if (isMedia && !mek.message.videoMessage || isQuotedImage) {
						...
						for (let _ of anu) {
							sendMess(_.jid, `[ *DIDIK BOT BROADCAST* ]\n\n${body.slice(4)}`)
						}
						reply('Suksess broadcast')
					}
					break
